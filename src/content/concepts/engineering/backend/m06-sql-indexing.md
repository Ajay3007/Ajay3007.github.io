...
description: "Backend Engineering · Phase 2 · Module 6 SQL Indexing The difference between a query that takes 2 ms and one that takes 2 minutes is almost always an index — or the lack of one."
domain: engineering
track: backend
order: 6
ownHeader: true
...
SELECT * FROM jobs
WHERE status = 'pending' AND created_at '5 minutes';
```
...
int get_user_by_id(PGconn *conn, int user_id) {
    ...
    for (int r = 0; r for (int c = 0; c "  %s = %s\n",
                   PQfname(res, c),
...
PGconn *pool_acquire(void) {
    ...
    for (int i = 0; i if (!pool[i].in_use && PQstatus(pool[i].conn) == CONNECTION_OK) {
            pool[i].in_use = 1;
...
void pool_release(PGconn *conn) {
    pthread_mutex_lock(&pool_lock);
    for (int i = 0; i if (pool[i].conn == conn) {
            pool[i].in_use = 0;
            break;
        }
    }
