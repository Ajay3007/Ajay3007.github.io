...
<link rel="stylesheet" href="{{ "/assets/css/sd-module-b7.css" | relative_url }}">
...
<pre class="c"><span class="cm">// 1. HTTP Upgrade handshake</span>
GET /ws HTTP/1.1
Upgrade: websocket
Connection: Upgrade
...
<span class="cm">// 5. Update last_read_message_id = latest received</span>
...
CREATE TABLE message_receipts (
    message_id    BIGINT,
    recipient_id  BIGINT,
    status        VARCHAR,  <span class="cm">-- 'delivered' | 'read'</span>
    updated_at    TIMESTAMP,
    PRIMARY KEY (message_id, recipient_id)
);
<span class="cm">-- Query: SELECT COUNT(*) FROM message_receipts</span>
<span class="cm">--        WHERE message_id = X AND status = 'read'</span>
<span class="cm">-- Compare to group size → determine if all have read</span></pre>
  </div>
...
<pre class="c"><span class="kw">public void</span> <span class="fn">handleGroupMessage</span>(GroupMessageEvent e) {
    ...
    <span class="kw">for</span> (<span class="kw">long</span> memberId : members) {
        ...
        <span class="kw">if</span> (serverAddr != <span class="kw">null</span>) {
            ...
            <span class="cm">// Offline: store in inbox for later delivery</span>
            redis.<span class="fn">zadd</span>(<span class="str">"inbox:"</span> + memberId, messageId, messageId);
        }
    }
}</pre>
  </div>
...
<div class="view" id="view-checklist">
  ...
  <div class="mb7-bottom-nav">
    ...
    <a href="/learning/system-design/hld/" class="mb7-nav-footer-btn">
...
<script src="{{ "/assets/js/sd-module-b7.js" | relative_url }}"></script>
