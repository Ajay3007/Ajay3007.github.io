...
<span class="ck">void</span> parse_udp(const uint8_t *ip_payload, uint16_t ip_payload_len) {
    ...
    <span class="ck">switch</span> (dst_port) {
        ...
        <span class="ck">case</span> 67:  handle_dhcp(payload, data_len); <span class="ck">break</span>;
        <span class="ck">case</span> 123: handle_ntp(payload, data_len);  <span class="ck">break</span>;
...
u16 dst_port = clib_net_to_host_u16(udp->dst_port);</pre></div>
  </div>
</div>
</div>
...
<div class="cb"><pre><span class="cm">/* UDP pseudo-session in NGFW conntrack */</span>
<span class="ck">typedef struct</span> {
    ...
    uint64_t       bytes_rev;            <span class="cm">/* server → client bytes */</span>
...
DHCP:         30 seconds
NTP:          30 seconds
VoIP/RTP:    180 seconds   <span class="cm">/* ongoing media stream */</span>
Generic UDP:  30 seconds   <span class="cm">/* catch-all default */</span></pre></div>
...
<div class="cb"><pre><span class="cm">/* ICMP Echo Request (Type 8, Code 0) */</span>
Type:       8
Code:       0
Checksum:   [computed]
...
<span class="cm"># TTL=245 → started at 255 (router)      → 10 hops away</span></pre></div>
  </div>
</div>
</div>
...
<div class="cb"><pre><span class="cm">/* Linux traceroute algorithm (UDP mode, default) */</span>
for ttl in 1..max_hops:
    send 3 UDP packets:
        IP: TTL = ttl, dst = target
        UDP: dst_port = 33434 + (ttl-1)*3  <span class="cm"># incrementing port per probe</span>
    wait for response:
...
Hop 7: 192.168.x.x    <span class="cm"># private address — NAT or misconfigured router</span>
RTT spike at hop 8     <span class="cm"># congestion at or beyond hop 8</span>
...
<!-- ── MODULE NAV ── -->
<div class="mod-nav">
  <a href="{{ '/learning/networking-mastery/m05-tcp/' | relative_url }}">← M05 TCP</a>
  <a href="{{ '/learning/networking-mastery/' | relative_url }}">🗺️ Roadmap</a>
  <a class="nb" href="{{ '/learning/networking-mastery/m07-dns/' | relative_url }}">Next: M07 - DNS →</a>
</div>
