"""
AXIOBYTE SYSTEMS -- Episode 02  (LANDSCAPE / YouTube long-form cut)
"Zero-copy: a packet is just a pointer"   (217.8 s, 16:9 landscape, 1920x1080)

This is the 16:9 sibling of ep02_video.py (the 9:16 portrait cut). SAME content,
SAME narration, SAME cue table and timings -- every scene is simply re-flowed for
a wide frame: the vertical flows of the portrait cut run left->right here, stacked
stages become a row, and the side-by-side panels are spread out. The drawing
toolkit, timeline controller, palette and pinned chrome are identical to the
portrait file; only the canvas constants and the per-act LAYOUT differ.

Render (video):   manim -qh --fps 60 ep02_video_16x9.py FullVideo
Render (draft):   manim -ql --fps 30 ep02_video_16x9.py FullVideo

Layout checks (one still frame each -- no timing, just geometry):
    manim -sqh ep02_video_16x9.py StillCopyChain    # NIC -> A -> B -> C, the copies
    manim -sqh ep02_video_16x9.py StillMbuf          # the mbuf, and what it points at
    manim -sqh ep02_video_16x9.py StillMempool       # the pool of preallocated buffers
    manim -sqh ep02_video_16x9.py StillPipeline      # four stages, one buffer

Act previews (each seeds the state it inherits and starts near its own t=0):
    manim -pql ep02_video_16x9.py PrevTrad       # 14 - 42   the copy tax
    manim -pql ep02_video_16x9.py PrevMbuf       # 42 - 68   zero-copy, and the mbuf
    manim -pql ep02_video_16x9.py PrevPool       # 68 -104   mempool + DMA
    manim -pql ep02_video_16x9.py PrevPipe       # 104-143   passing the handle
    manim -pql ep02_video_16x9.py PrevCheap      # 143-184   why a pointer wins
    manim -pql ep02_video_16x9.py PrevClose      # 184-end   transmit, recycle, recap
...
# ---------------------------------------------------------------------------
# CANVAS -- 1920 x 1080 design space, 1 unit == 100 px  (16:9 landscape)
# ---------------------------------------------------------------------------
config.pixel_width = 1920
config.pixel_height = 1080
config.frame_width = 19.2
config.frame_height = 10.8

DW, DH = 1920, 1080
CXP, CYP = DW / 2, DH / 2               # 960, 540
FH = DH / 100.0                         # full frame height in manim units (10.8)
...
def act_hook(scene, st, kb, cap):
    ...
    # the library sits on the LEFT; everything will be hauled RIGHT to a new building.
    lib = VGroup(*[shelf(430, 360 + i * 96) for i in range(4)]).set_z_index(Z_NODE)
    ...
    libtag = tag("the library  =  your packet's bytes", MEM, TAG_T - 2).move_to(P(430, 270))
    scene.at("hk_library")
    ...
    truck = VGroup(truck, wheels).move_to(P(430, 540)).set_z_index(Z_NODE)
    truck.set_opacity(0.0)
    scene.at("hk_pack")
    scene.play(FadeOut(halo), FadeOut(booktag), FadeOut(libtag),
               lib.animate.scale(0.42).move_to(P(430, 540)), run_time=0.7)
    scene.at("hk_truck")
    scene.play(truck.animate.set_opacity(1.0), lib.animate.set_opacity(0.0), run_time=0.4)
    scene.remove(lib)

    # 6.86  "DRIVE them to a NEW BUILDING, unload"  -- across the WHOLE wide frame it
    # goes, and everything it carried is set down again, untouched, for one read.
    dest = box(300, 380, IDLE, fill=SURF2, r=0.06, sw=2.4).move_to(P(1500, 540))
    destt = M("new building", MONO_S, IDLE).next_to(dest, UP, buff=U(10))
    scene.at("hk_drive")
    scene.play(FadeIn(dest), FadeIn(destt), run_time=0.35)
    scene.play(truck.animate.move_to(P(1280, 540)), run_time=0.9, rate_func=rush_from)
    lib2 = VGroup(*[box(240, 62, PKT, fill=SURF2, r=0.03, sw=1.6).move_to(P(1500, 430 + i * 78))
    ...
    scene.at("hk_repeat")
    loop = CurvedArrow(P(1320, 320), P(560, 320), angle=-TAU / 3.5, color=BAD, stroke_width=5) \
        .set_z_index(Z_LINE)
    ...
    stamp = pill("ridiculously inefficient", BAD, BAD_BG, TAG_T, h=64, glowing=True) \
        .move_to(P(CXP, 860)).rotate(-6 * DEGREES)
...
def act_traditional(scene, st, kb, cap):
    ...
    scene.at("tr_thats")
    pkt0 = packet_buffer(CXP, 520, n=6, w=300, h=150, title="", glowing=True)
    ...
    # 18.37  the three tiers the packet must cross, as columns LEFT->RIGHT: hardware
    # -> kernel -> app. Each boundary between them is where a copy will happen.
    hw = node("HARDWARE", "NIC", 300, 200, NIC_C, 420, 520, tsize=NODE_T - 3)
    krn = node("OS KERNEL", "network stack", 300, 200, KERN, 960, 520, tsize=NODE_T - 3)
    app = node("APPLICATION", "your process", 300, 200, PKT, 1500, 520, tsize=NODE_T - 3)
    scene.at("tr_networking")
    scene.play(FadeOut(pkt0, shift=RIGHT * 0.3),
               LaggedStart(FadeIn(hw, shift=RIGHT * 0.1), FadeIn(krn), FadeIn(app, shift=LEFT * 0.1),
    ...
    trav = packet_buffer(280, 520, n=4, w=150, h=90, title="", glowing=True)
    trav.title.set_opacity(0.0)
    scene.at("tr_travels")
    scene.add(trav)
    scene.play(Indicate(hw, color=NIC_C, scale_factor=1.05), run_time=0.4)
    scene.at("tr_hardware")
    scene.play(trav.animate.move_to(P(420, 520)), run_time=0.4)
    scene.at("tr_kernel")
    scene.play(trav.animate.move_to(P(960, 520)),
               Indicate(krn, color=KERN, scale_factor=1.04), run_time=0.5)
    scene.at("tr_application")
    scene.play(trav.animate.move_to(P(1500, 520)),
    ...
    b1 = DashedLine(P(690, 400), P(690, 640), color=BAD, stroke_width=3, dash_length=0.14) \
        .set_z_index(Z_LINE)
    b2 = DashedLine(P(1230, 400), P(1230, 640), color=BAD, stroke_width=3, dash_length=0.14) \
        .set_z_index(Z_LINE)
    scene.at("tr_copied")
    scene.play(Create(b1), Create(b2), run_time=0.5)
    scene.at("tr_boundaries")
    ghost1 = byte_cells(4, color=BAD).scale(0.5).move_to(P(690, 360))
    ghost2 = byte_cells(4, color=BAD).scale(0.5).move_to(P(1230, 360))
    scene.play(FadeIn(ghost1, target_position=P(690, 520)),
               FadeIn(ghost2, target_position=P(1230, 520)), run_time=0.5)
    scene.play(FadeOut(ghost1), FadeOut(ghost2), run_time=0.3)
    st.bounds = VGroup(b1, b2)

    # 27.24  Now the real chain: the buffers, LEFT->RIGHT.  NIC -> A -> B -> C. This
    # is the sequence the whole film exists to demolish, so it is drawn honestly.
    kb.to("THREE BUFFERS, THREE COPIES", BAD, run_time=0.5,
          extra=[FadeOut(hw, shift=LEFT * 0.2), FadeOut(krn), FadeOut(app, shift=RIGHT * 0.2),
                 FadeOut(st.bounds)])
    nic = node("NIC", "off the wire", 280, 120, NIC_C, 290, 540, tsize=NODE_T)
    bufA = packet_buffer(700, 540, n=6, w=320, h=150, title="buffer A", glowing=True)
    scene.at("nc_nic")
    scene.play(FadeIn(nic, shift=RIGHT * 0.15), run_time=0.45)
    scene.at("nc_receives")
    nicarr = vlink(P(432, 540), P(538, 540), color=NIC_C)
    ...
    # 31.86  "copied into ANOTHER BUFFER for processing"  A -> B. A red copy of every
    # byte physically slides RIGHT into a new buffer. The bytes MOVED.
    bufB = packet_buffer(1130, 540, addr="0x9c14e880", n=6, w=320, h=150,
                         title="buffer B", glowing=True)
    bufB.cells.set_opacity(0.0)
    arrAB = vlink(P(862, 540), P(968, 540), color=BAD, sw=6)
    scene.at("c1_copied")
    ...
    bufC = packet_buffer(1560, 540, addr="0xab02f100", n=6, w=320, h=150,
                         title="buffer C  (tx)", glowing=True)
    bufC.cells.set_opacity(0.0)
    arrBC = vlink(P(1292, 540), P(1398, 540), color=BAD, sw=6)
    scene.at("c2_copied")
    ...
    # Three meters in a row below the chain, filling red as each cost is named.
    mCPU = cost_meter("CPU cycles", 500, 800, color=BAD)
    mBW = cost_meter("memory bandwidth", 960, 800, color=BAD)
    mCA = cost_meter("cache", 1420, 800, color=BAD)
    scene.at("ev_every")
    scene.play(LaggedStart(FadeIn(mCPU), FadeIn(mBW), FadeIn(mCA), lag_ratio=0.2),
               run_time=0.7)
    scene.at("ev_cpu")
...
def act_zerocopy_intro(scene, st, kb, cap):
    ...
    set_cam(scene, FULL_FRAME)
    buf = packet_buffer(640, 560, n=6, w=360, h=180, glowing=True)
    scene.at("in_instead")
    scene.play(FadeIn(buf, scale=1.08), run_time=0.55)
    nocopy = cross(P(640, 320), s=90)
    nocopy_t = tag("no copy", BAD, TAG_T - 2).move_to(P(640, 400))
    ...
    ref = ptr_arrow(P(1180, 560), P(830, 560))
    ref_t = tag("a reference", PTR, TAG_T - 2).move_to(P(1250, 465))
    ...
    zc = pill("ZERO COPY", MBUF_C, MBUF_BG, NODE_T, h=96, glowing=True).move_to(P(CXP, 300))
    scene.at("zc_zero")
...
def act_mempool(scene, st, kb, cap):
    ...
    pool = mempool(CXP, 560, cols=6, rows=4, slot=120, gap=18)
    pool.title.set_opacity(0.0)
    scene.at("ml_dpdk")
    dpdk = tag("DPDK · at startup", MBUF_C, TAG_T - 2).move_to(P(CXP, 270))
    ...
    ml = pill("mempool", MEM, MEM_BG, NODE_T - 2, h=76, glowing=True).move_to(P(CXP, 270))
    ...
    kb.to("DMA — STRAIGHT TO MEMORY", NIC_C, run_time=0.5,
          extra=[FadeOut(ml, shift=UP * 0.2),
                 pool.animate.scale(0.72).move_to(P(1220, 540))])
    nic = node("NIC", "100 GbE", 300, 150, NIC_C, 340, 540, tsize=NODE_T)
    scene.at("dm_nic")
    scene.play(FadeIn(nic, shift=RIGHT * 0.15), run_time=0.45)
    scene.at("dm_new")
    malloc = tag("malloc() a new buffer?", KERN, TAG_T - 3).move_to(P(720, 340))
    ...
    scene.at("dm_dma")
    dma_t = pill("DMA engine", PTR, PTR_BG, TAG_T, h=64, glowing=True).move_to(P(720, 400))
    scene.play(FadeIn(dma_t, scale=1.1), Indicate(nic, color=NIC_C, scale_factor=1.04),
               run_time=0.5)
    scene.at("dm_stream")
    stream = byte_cells(6, color=PTR).scale(0.8).move_to(P(560, 540))
    scene.play(FadeIn(stream, target_position=nic.get_right()), run_time=0.4)
    scene.at("dm_wire")
    path = ptr_arrow(P(620, 540), target.get_left() + LEFT * 0.05, sw=6)
    ...
    # Three denials on the left, each struck through as spoken, while the one
    # blue-filled slot stays on the right -- every clause negates a copy-path cost.
    denies = VGroup(
        M("CPU involvement", ROW_T, SUBTLE),
        M("dynamic allocation", ROW_T, SUBTLE),
        M("unnecessary copies", ROW_T, SUBTLE),
    ).arrange(DOWN, buff=U(48), aligned_edge=LEFT).move_to(P(640, 540))
    nos = VGroup(*[T("no", ROW_T, BAD).next_to(d, LEFT, buff=0.2) for d in denies])
    scene.play(FadeOut(st.dma_t), FadeOut(nic, shift=LEFT * 0.2), run_time=0.5)
    strikes = VGroup()
...
def act_pipeline(scene, st, kb, cap):
    ...
    buf = packet_buffer(960, 720, n=6, w=360, h=170, glowing=True)
    # title BELOW the box -- the stage pointers come down onto the box top, so a
    # title above it would sit under the arrowheads.
    buf.title.become(M("the one buffer", MONO_S, MEM).next_to(buf.box, DOWN, buff=U(14)))
    scene.play(FadeIn(buf, scale=1.08), run_time=0.5)
    st.buf = buf

    # the four stages, as a ROW across the top; the one buffer sits below them.
    stages = []
    names = [("PARSER", "s_parser"), ("CLASSIFIER", "s_classifier"),
             ("FIREWALL", "s_firewall"), ("ROUTING", "s_routing")]
    xs = [360, 720, 1080, 1440]
    for (nm, _), x in zip(names, xs):
        stages.append(node(nm, None, 300, 130, MBUF_C, x, 360, tsize=NODE_T - 4,
                           glowing=False))
    stage_grp = VGroup(*stages)
    scene.at("pl_stage")
    scene.play(LaggedStart(*[FadeIn(s, shift=DOWN * 0.12) for s in stages],
    ...
    # 109.64  The mbuf handle visits each stage in turn -- the chip slides RIGHT
    # along the row while the yellow pointer re-anchors its TAIL to the chip and
    # keeps its HEAD welded to the one buffer below. Only the arrow ever moves.
    chip = pill("mbuf", MBUF_C, MBUF_BG, TAG_T, h=54, glowing=True)
    ptr = [None]

    def hand_to(stage, first=False):
        chip.generate_target()
        chip.target.next_to(stage, DOWN, buff=0.18)
        new_ptr = ptr_arrow(chip.target.get_bottom() + DOWN * 0.05,
                            buf.box.get_top() + UP * 0.05)
        if first:
            chip.next_to(stage, DOWN, buff=0.18)
            scene.add(chip)
    ...
    # once, all coming down to the one buffer.
    scene.at("pl_each")
    fan = VGroup(*[ptr_arrow(s.get_bottom() + DOWN * 0.05, buf.box.get_top() + UP * 0.05,
    ...
    scene.at("pl_points")
    scene.play(Flash(buf.box.get_top(), color=PTR, line_length=0.16, num_lines=12,
    ...
    scene.at("nm_never")
    lock = tag("pinned · never moves", MEM, TAG_T - 2).next_to(buf, DOWN, buff=0.22)
...
def act_cheap(scene, st, kb, cap):
    ...
    scene.play(VGroup(st.buf.cells[0], st.buf.cells[1]).animate.set_color(PKT).set_fill(PKT, 0.85),
               st.buf.animate.move_to(P(CXP, 520)), run_time=0.6)
    ...
    scene.at("ky_only")
    p = ptr_arrow(P(920, 300), st.buf.box.get_top() + UP * 0.05)
    ...
    smallg = VGroup(small_lab, small).move_to(P(560, 540))
    ...
    scene.at("ch_copying")
    big_cells = VGroup(*[RoundedRectangle(width=U(26), height=U(34), corner_radius=U(2),
                                          stroke_color=PKT, stroke_width=0.8,
                                          fill_color=PKT, fill_opacity=0.8)
                         for _ in range(60)]).arrange_in_grid(rows=6, cols=10, buff=U(5))
    big_lab = T("1500 bytes", ROW_T, PKT).next_to(big_cells, UP, buff=0.2)
    bigg = VGroup(big_lab, big_cells).move_to(P(1360, 540))
    vs = T("vs", NODE_T, IDLE).move_to(P(CXP, 540))
    ...
    # TIME MOVING BYTES instead of FORWARDING." One wide bar of the core's time:
    # almost all of it red (memcpy), a sliver blue (the actual work).
    core = box(1000, 130, KERN, fill=SURF2, r=0.1, sw=2.6).move_to(P(CXP, 660))
...
def act_payoff(scene, st, kb, cap):
    ...
    core_box = st.core[0]
    red_small = Rectangle(width=U(1000) * 0.12, height=U(112), stroke_width=0,
                          fill_color=BAD, fill_opacity=0.85)
    ...
    if getattr(st, "fwd", None) is not None:
        blue_big = Rectangle(width=U(1000) * 0.82, height=U(112), stroke_width=0,
                             fill_color=PKT, fill_opacity=0.85)
    ...
    # three benefit chips in a ROW (the reclaimed-core bar fades as they enumerate),
    # then the throughput gauge.
    chips = [("↓  CPU overhead", "po_reduces", KERN),
             ("↓  memory bandwidth", "po_lowers", MEM),
             ("↑  cache efficiency", "po_improves", PKT)]
    xs = [500, 960, 1420]
    built = []
    for i, ((txt, key, col), x) in enumerate(zip(chips, xs)):
        scene.at(key)
        c = pill(txt, col, SURF2, TAG_T, h=68, glowing=True).move_to(P(x, 400))
        extra = [FadeOut(st.core, scale=0.9)] if i == 0 else []
        scene.play(FadeIn(c, shift=UP * 0.12), *extra, run_time=0.45)
        built.append(c)

    # 178.31  "applications like DPDK ... TENS or HUNDREDS of GIGABITS per second."
    scene.at("po_dpdk")
    dpdk = pill("DPDK", MBUF_C, MBUF_BG, NODE_T - 2, h=76, glowing=True).move_to(P(CXP, 590))
    scene.play(FadeIn(dpdk, scale=1.12), run_time=0.4)
    gauge_bg = box(1000, 66, IDLE, fill=SURF2, r=0.33, sw=2.0).move_to(P(CXP, 730))
    gfill = Rectangle(width=U(1000) * 0.45, height=U(50), stroke_width=0,
    ...
    gfill_full = Rectangle(width=U(1000) * 0.95, height=U(50), stroke_width=0,
                           fill_color=NIC_C, fill_opacity=0.9)
    gfill_full.move_to(gauge_bg.get_left(), aligned_edge=LEFT).shift(RIGHT * 0.07) \
        .set_z_index(Z_NODE)
    num = T("100+ Gb/s", NODE_T, NIC_C).move_to(P(CXP, 850))
    scene.play(Transform(gfill, gfill_full), FadeIn(num, scale=1.15),
               Flash(P(CXP, 730), color=NIC_C, line_length=0.2, num_lines=16,
...
def act_tx(scene, st, kb, cap):
    ...
    buf = packet_buffer(720, 500, n=6, w=340, h=170, glowing=True)
    nic = node("NIC", "tx", 300, 130, NIC_C, 1400, 500, tsize=NODE_T)
    scene.at("tx_complete")
    scene.play(FadeIn(nic, shift=LEFT * 0.12), FadeIn(buf, scale=1.05), run_time=0.6)
    ...
    p = ptr_arrow(buf.box.get_right() + RIGHT * 0.05, nic.get_left() + LEFT * 0.05)
    scene.play(GrowArrow(p), run_time=0.45)
    wire = byte_cells(6, color=PKT).scale(0.6).move_to(buf.cells.get_center())
    scene.add(wire)
    scene.at("tx_same")
    scene.play(wire.animate.move_to(P(1790, 500)).scale(0.7), run_time=0.7, rate_func=rush_from)
    scene.at("tx_buffer")
    ...
    scene.at("rc_after")
    pool = mempool(CXP, 830, cols=6, rows=2, slot=90, gap=14, hot=8)
    scene.play(FadeOut(p), FadeIn(pool, shift=UP * 0.1), run_time=0.6)
    scene.at("rc_returned")
    scene.play(buf.animate.scale(0.28).move_to(pool.slots[8].get_center()),
               FadeOut(nic, shift=RIGHT * 0.2), run_time=0.8, rate_func=rush_from)
    scene.at("rc_mempool")
...
def act_recap(scene, st, kb, cap):
    ...
    # 201.69  "ONLY the mBuff, the HANDLE, was PASSED." The mbuf chip slides RIGHT
    # across the whole frame -- the only thing that ever moved.
    scene.at("rp_only")
    line3 = M("only the mbuf was passed", ROW_T, INK).move_to(P(CXP, 620))
    scene.play(FadeIn(line3, shift=RIGHT * 0.1), run_time=0.4)
    chip = pill("mbuf", MBUF_C, MBUF_BG, TAG_T - 2, h=50, glowing=True).move_to(P(380, 720))
    scene.at("rp_mbuf")
    scene.play(FadeIn(chip, scale=1.1), run_time=0.35)
    scene.at("rp_handle")
    trail = ptr_arrow(P(430, 770), P(1490, 770), sw=5).set_opacity(0.6)
    scene.play(GrowArrow(trail), run_time=0.4)
    scene.at("rp_passed")
    scene.play(chip.animate.move_to(P(1480, 720)), run_time=0.9, rate_func=rush_from)
    st.recap = VGroup(*made, line3, chip, trail)

    # 207.04  THE THESIS. A blue block of bytes on the LEFT, labelled "packet"; the
    # statements on the RIGHT. We strike out "= bytes" and reveal "= a pointer"
    # (yellow), whose arrow points straight back at the bytes. The film in one line.
    scene.at("fn_thats")
    kb.to("A PACKET IS…", PTR, run_time=0.5, extra=[FadeOut(st.recap, scale=0.85)])
    bytes_block = VGroup(byte_cells(6, cw=48, ch=70, color=PKT),
                         M("the bytes", MONO_S, PKT)).arrange(DOWN, buff=U(14)) \
        .move_to(P(600, 470))
    scene.at("fn_packet")
    scene.play(FadeIn(bytes_block, scale=1.08), run_time=0.5)
    eq1 = T("packet  =  a pile of bytes", NODE_T - 4, INK).move_to(P(1220, 410))
    scene.at("fn_not")
    scene.play(FadeIn(eq1), run_time=0.4)
    scene.at("fn_bytes1")
    scene.play(Create(strike(eq1[-11:] if len(eq1) > 11 else eq1, color=BAD)),
               run_time=0.45)
    # the substitution: bytes -> pointer, its arrow aimed back at the bytes
    scene.at("fn_pointer")
    eq2 = T("packet  =  a pointer", NODE_T - 2, PTR).move_to(P(1220, 560))
    arrow = ptr_arrow(eq2.get_left() + LEFT * 0.1, bytes_block.get_right() + RIGHT * 0.05)
    scene.play(FadeIn(eq2, shift=UP * 0.1), GrowArrow(arrow),
               Flash(eq2.get_center(), color=PTR, line_length=0.16, num_lines=12,
...
def act_outro(scene, st):
    ...
    brand = VGroup(row, sysx).move_to(P(CXP, 300))

    head = T("High-Performance Data Plane", NODE_T + 2, INK).move_to(P(CXP, 490))
    if head.width > 12.0:
        head.scale_to_fit_width(12.0)
    sub = T("systems, from first principles", CAP_SIZE, IDLE, bold=False).move_to(P(CXP, 560))
    dl = Line(LEFT * 1.6, ORIGIN, color=MBUF_C, stroke_width=4).move_to(P(CXP - 80, 630))
    dr = Line(ORIGIN, RIGHT * 1.6, color=KERN, stroke_width=4).move_to(P(CXP + 80, 630))
    ...
    recap = VGroup(T("this episode ↑", CAP_SIZE + 2, PTR),
        ...
        .arrange(RIGHT, buff=0.25).move_to(P(CXP, 900))
    if recap.width > 12.0:
        recap.scale_to_fit_width(12.0)
...
class PrevPool(_Preview):
    ...
    def body(self):
        self.st.buf = packet_buffer(640, 560, glowing=True)
        self.st.mbuf = mbuf_card(1300, 560)
...
class PrevPipe(_Preview):
    ...
    def body(self):
        ...
        self.st.pool = mempool(1220, 540, cols=6, rows=4, slot=88, gap=12, hot=14).scale(0.72)
...
class PrevCheap(_Preview):
    ...
    def body(self):
        self.st.buf = packet_buffer(960, 720, glowing=True)
        self.st.stages = VGroup(*[node(n, None, 300, 130, MBUF_C, x, 360, glowing=False)
                                  for n, x in zip(("PARSER", "CLASSIFIER", "FIREWALL", "ROUTING"),
                                                  (360, 720, 1080, 1440))])
        ...
        self.st.fan = VGroup(*[ptr_arrow(s.get_bottom(), self.st.buf.box.get_top())
...
class StillCopyChain(Scene):
    def construct(self):
        self.camera.background_color = BG
        nic = node("NIC", "off the wire", 280, 120, NIC_C, 290, 540)
        a = packet_buffer(700, 540, w=320, h=150, title="buffer A")
        b = packet_buffer(1130, 540, w=320, h=150, addr="0x9c14e880", title="buffer B")
        c = packet_buffer(1560, 540, w=320, h=150, addr="0xab02f100", title="buffer C  (tx)")
        self.add(nic, a, b, c,
                 vlink(P(432, 540), P(538, 540), color=NIC_C),
                 vlink(P(862, 540), P(968, 540), color=BAD, sw=6),
                 vlink(P(1292, 540), P(1398, 540), color=BAD, sw=6),
                 cost_meter("CPU cycles", 500, 800), cost_meter("memory bandwidth", 960, 800),
                 cost_meter("cache", 1420, 800),
...
class StillMbuf(Scene):
    def construct(self):
        self.camera.background_color = BG
        buf = packet_buffer(640, 560, w=360, h=180, glowing=True)
        mb = mbuf_card(1300, 560)
...
class StillMempool(Scene):
    def construct(self):
        self.camera.background_color = BG
        self.add(mempool(CXP, 560, cols=6, rows=4, slot=120, gap=18, hot=14),
                 T("THE MEMPOOL", KEY_SIZE, MEM).move_to(P(CXP, 190)))


class StillPipeline(Scene):
    def construct(self):
        self.camera.background_color = BG
        buf = packet_buffer(960, 720, w=360, h=170, glowing=True)
        buf.title.next_to(buf.box, DOWN, buff=U(14))
        stages = [node(n, None, 300, 130, MBUF_C, x, 360, glowing=False)
                  for n, x in zip(("PARSER", "CLASSIFIER", "FIREWALL", "ROUTING"),
                                  (360, 720, 1080, 1440))]
        fan = [ptr_arrow(s.get_bottom() + DOWN * 0.05, buf.box.get_top() + UP * 0.05, sw=3)
               for s in stages]
        self.add(buf, *stages, *fan,
                 T("ONE BUFFER, MANY STAGES", KEY_SIZE, MEM).move_to(P(CXP, 180)))
