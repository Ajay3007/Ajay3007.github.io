...
class Cover45(Scene):
    def construct(self):
        ...
        self.add(VGroup(pill("# 45",CYAN,SURFACE,CYAN),pill("Medium",MED_T,MED_B,MED_T)).arrange(RIGHT,buff=0.28).move_to(UP*5.1))
        title=Text("Jump Game II",font=FN,weight=BOLD,color=WHITE)
        title.scale_to_fit_width(5.6).move_to(UP*3.95)
        self.add(title)
        ...
        sub=Text("fewest jumps to reach the end",font=MN,color=GRAY).scale(0.36).move_to(UP*2.35)
        self.add(sub)
        vals=[2,3,1,1,4]
        row=VGroup()
        for i,v in enumerate(vals):
            if i in (0,1,4): row.add(mcell(v,MINT,MINT_BG,WHITE,0.7))
            else: row.add(mcell(v,BORDER,SURFACE,WHITE,0.7))
        row.arrange(RIGHT,buff=0.18).move_to(UP*1.35)
        self.add(row,Text("nums",font=MN,weight=BOLD,color=WHITE).scale(0.36).next_to(row,LEFT,buff=0.22))
        idx=VGroup(*[Text(str(i),font=MN,color=GRAY).scale(0.32).next_to(row[i],DOWN,buff=0.16) for i in range(5)])
        self.add(idx)
        a1=CurvedArrow(row[0].get_top()+UP*0.03,row[1].get_top()+UP*0.03,angle=-PI*0.6,color=MINT,stroke_width=4,tip_length=0.16)
        a2=CurvedArrow(row[1].get_top()+UP*0.03,row[4].get_top()+UP*0.03,angle=-PI*0.5,color=MINT,stroke_width=4,tip_length=0.16)
        self.add(a1,a2)

        hook=Text("greedy  \u00b7  expand the frontier in levels",font=FN,color=CYAN).scale(0.44)
        if hook.width>6.6: hook.scale_to_fit_width(6.6)
        hook.move_to(UP*-0.55); self.add(hook)

        box=RoundedRectangle(width=4.0,height=1.2,corner_radius=0.18,stroke_color=MINT,stroke_width=3.2,fill_color=MINT_BG,fill_opacity=1.0).move_to(UP*-1.95)
        bval=Text("min jumps = 2",font=MN,weight=BOLD,color=MINT).scale(0.6).move_to(box.get_center())
        self.add(box,bval)
        self.add(Text("0 \u2192 1 \u2192 4",font=MN,color=GRAY).scale(0.4).next_to(box,DOWN,buff=0.24))

        self.add(Text("O(n) time      \u00b7      O(1) space",font=MN,weight=BOLD,color=WHITE).scale(0.44).move_to(UP*-4.05))
