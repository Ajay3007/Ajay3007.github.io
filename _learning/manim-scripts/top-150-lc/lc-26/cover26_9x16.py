...
class Cover26(Scene):
    def construct(self):
        ...
        self.add(VGroup(pill("# 26",CYAN,SURFACE,CYAN),pill("Easy",GREEN_T,GREEN_B,GREEN_T)).arrange(RIGHT,buff=0.28).move_to(UP*5.1))
        title=Text("Remove Duplicates",font=FN,weight=BOLD,color=WHITE)
        ...
        # input: sorted, duplicates red
        note=Text("red = duplicate",font=MN,color=RED).scale(0.36).move_to(UP*2.25)
        self.add(note)
        ...
        hook=Text("sorted  \u2192  duplicates are adjacent",font=FN,color=CYAN).scale(0.48)
        if hook.width>6.4: hook.scale_to_fit_width(6.4)
        hook.move_to(UP*-0.25); self.add(hook)

        res=VGroup(mcell(1,MINT,MINT_BG,MINT,0.92),mcell(2,MINT,MINT_BG,MINT,0.92),mcell(3,MINT,MINT_BG,MINT,0.92),
                   mempty(0.92),mempty(0.92)).arrange(RIGHT,buff=0.12).move_to(UP*-1.6)
        self.add(res)

        self.add(VGroup(pill("k = 3",MINT,SURFACE,MINT,s=0.4),Text("unique values",font=MN,color=GRAY).scale(0.4)
