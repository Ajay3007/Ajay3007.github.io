...
MAX_Y=2.7; BASE_Y=-1.05; UH=0.32; BW=0.6; GAP=0.06; STEPX=0.66; CODE_Y=-3.55
READ=0.8; READ_L=1.4

H=[1,8,6,2,5,4,8,3,7]; N=len(H)
...
class LC11(Scene):
    ...
    def persistent(self):
        ...
        badge=pill("# 11",CYAN,SURFACE,CYAN,s=0.4)
        title=Text("Container With Most Water",font=FN,weight=BOLD,color=WHITE).scale(0.6)
    ...
    # ---------- v3 code panel ----------
    def build_code(self,raw,scale=0.30,cy=CODE_Y,maxw=6.0):
    ...
    def cx(self,i):
        x0=-((N-1)*STEPX)/2
        return x0+i*STEPX
    def top(self,i): return BASE_Y+H[i]*UH

    def make_bar(self,i):
        r=Rectangle(width=BW,height=H[i]*UH,fill_color=TERR,fill_opacity=1.0,stroke_color=BORDER,stroke_width=2.0)
        r.move_to([self.cx(i),BASE_Y+H[i]*UH/2,0]).set_z_index(3); return r

    def water(self,l,r,lvl):
        xL=self.cx(l); xR=self.cx(r); y1=BASE_Y+lvl*UH
        w=Rectangle(width=xR-xL,height=y1-BASE_Y,fill_color=WATER,fill_opacity=0.42,stroke_color=WATER,stroke_width=1.4)
        w.move_to([(xL+xR)/2,(BASE_Y+y1)/2,0]).set_z_index(1); return w
    ...
    def problem_scene(self):
        ...
        t1=Text("Each bar is a vertical line.",font=FN,color=WHITE).scale(0.44)
        t2=Text("Pick two — which pair holds the most water?",font=FN,color=GRAY).scale(0.4)
        for t in (t1,t2):
        ...
        ground=Line([self.cx(0)-BW/2-0.15,BASE_Y,0],[self.cx(N-1)+BW/2+0.15,BASE_Y,0],color=GRAY,stroke_width=3)
        self.play(Create(ground),run_time=0.4)
        self.play(LaggedStart(*[GrowFromEdge(b,DOWN) for b in self.bars],lag_ratio=0.07),run_time=1.1)
        hv=VGroup(*[Text(str(H[i]),font=MN,color=GRAY).scale(0.3).next_to(self.bars[i],UP,buff=0.06) for i in range(N)])
        self.play(FadeIn(hv),run_time=0.4)
        f=Text("area = min(left, right) × width",font=MN,weight=BOLD,color=WATER).scale(0.42).move_to(UP*-2.6)
        if f.width>6.9: f.scale_to_fit_width(6.9)
        self.play(FadeIn(f,shift=UP*0.1),run_time=0.5)
        self.wait(READ_L)
        self.play(FadeOut(VGroup(lbl,t1,t2,self.bars,ground,hv,f)),run_time=0.5)

    def hint_scene(self):
        lbl=pill("The key idea",AMBER,SURFACE,AMBER).move_to(UP*CAP_Y)
        h1=Text("Start wide: one line at each end.",font=FN,weight=BOLD,color=WHITE).scale(0.46)
        h2=Text("Water is capped by the SHORTER line.",font=FN,color=WATER).scale(0.44)
        h3=Text("Moving the taller line can only shrink width…",font=FN,color=GRAY).scale(0.4)
        h4=Text("…so always move the shorter line inward.",font=FN,color=MINT).scale(0.42)
        h5=Text("It's the only move that might help.",font=FN,color=CYAN).scale(0.42)
        for h in (h1,h2,h3,h4,h5):
            if h.width>6.8: h.scale_to_fit_width(6.8)
        h1.move_to(UP*2.1); h2.next_to(h1,DOWN,buff=0.3)
        h3.next_to(h2,DOWN,buff=0.28); h4.next_to(h3,DOWN,buff=0.2); h5.next_to(h4,DOWN,buff=0.28)
        self.play(FadeIn(lbl,shift=UP*0.1),run_time=0.5)
        self.play(FadeIn(h1,shift=UP*0.1),run_time=0.5); self.wait(0.2)
        self.play(FadeIn(h2,shift=UP*0.1),run_time=0.42)
        self.play(FadeIn(h3,shift=UP*0.1),run_time=0.4); self.play(FadeIn(h4,shift=UP*0.1),run_time=0.42)
        self.play(FadeIn(h5,shift=UP*0.1),run_time=0.4)
    ...
    def approach_scene(self):
        ...
        a0=Text("Two pointers, shrink inward",font=FN,weight=BOLD,color=WHITE).scale(0.52).move_to(UP*2.3)
        a1=Text("l = 0,   r = n - 1,   best = 0",font=MN,color=WHITE).scale(0.4).move_to(UP*1.4)
        a2=Text("area = min(h[l], h[r]) × (r - l)",font=MN,color=WATER).scale(0.38).move_to(UP*0.7)
        a3=Text("best = max(best, area)",font=MN,color=MINT).scale(0.4).move_to(UP*0.1)
        a4=Text("h[l] < h[r] ? l++ : r--   (move the shorter)",font=MN,color=CYAN).scale(0.36).move_to(UP*-0.55)
        ...
        self.play(FadeIn(a3,shift=UP*0.1),run_time=0.4)
    ...
    def walkthrough(self):
        self.bars=VGroup(*[self.make_bar(i) for i in range(N)])
        ground=Line([self.cx(0)-BW/2-0.15,BASE_Y,0],[self.cx(N-1)+BW/2+0.15,BASE_Y,0],color=GRAY,stroke_width=3)
        idx=VGroup(*[Text(str(i),font=MN,color=GRAY).scale(0.28).move_to([self.cx(i),BASE_Y-0.32,0]) for i in range(N)])
        self.ground=ground; self.idx=idx
        self.play(Create(ground),LaggedStart(*[GrowFromEdge(b,DOWN) for b in self.bars],lag_ratio=0.05),FadeIn(idx),run_time=1.1)

        raw=[(0,"int l = 0, r = n - 1, best = 0;"),
             (0,"while (l < r) {"),
             (1,"int h = min(height[l], height[r]);"),
             (1,"best = max(best, h * (r - l));"),
             (1,"if (height[l] < height[r]) l++;"),
             (1,"else r--;"),
             (0,"}"),
             (0,"return best;")]
        ...
        l=0; r=N-1; best=0; bl=br=0
        self.pl=pointer("l",CYAN,up=False).next_to(self.idx[0],DOWN,buff=0.1)
        self.pr=pointer("r",AMBER,up=False).next_to(self.idx[N-1],DOWN,buff=0.1)
        self.maxchip=pill("max = 0",MINT,MINT_BG,MINT,s=0.44,h=0.62,glowing=True).move_to(UP*MAX_Y)
        self.play(FadeIn(self.pl,shift=UP*0.1),FadeIn(self.pr,shift=UP*0.1),FadeIn(self.maxchip,shift=UP*0.1),*self.hl(2),run_time=0.5)

        self.curwater=None; step=0
        while l<r:
            step+=1
            lvl=min(H[l],H[r]); area=lvl*(r-l)
            newbest=area>best
            detailed=(step<=2 or newbest)
            rt=0.5 if detailed else 0.32
            w=self.water(l,r,lvl)
            anims=[self.bars[l].animate.set_stroke(CYAN,4.2),self.bars[r].animate.set_stroke(AMBER,4.2),*self.hl(3)]
            if self.curwater is not None: anims.append(FadeOut(self.curwater))
            anims.append(FadeIn(w))
            self.set_cap("min(%d,%d) × %d = %d" % (H[l],H[r],r-l,area),
                         color=(MINT if newbest else WATER),scale=0.46)
            self.play(*anims,run_time=rt)
            self.curwater=w
            if newbest:
                best=area; bl,br=l,r
                nm=pill("max = %d"%best,MINT,MINT_BG,MINT,s=0.44,h=0.62,glowing=True).move_to(UP*MAX_Y)
                self.play(Transform(self.maxchip,nm),run_time=0.4)
            # move shorter side
            shortleft=H[l]<H[r]
            self.play(*self.hl(4 if shortleft else 5),
                      self.bars[l].animate.set_stroke(BORDER,2.0),self.bars[r].animate.set_stroke(BORDER,2.0),
                      run_time=0.28 if not detailed else 0.34)
            if shortleft:
                l+=1
                self.play(self.pl.animate.next_to(self.idx[l],DOWN,buff=0.1),run_time=0.26)
            else:
                r-=1
                self.play(self.pr.animate.next_to(self.idx[r],DOWN,buff=0.1),run_time=0.26)
        self.best=best; self.bl=bl; self.br=br
        self.wait(0.3)

    def outro(self):
        # redraw the winning container with a strong glow
        if self.curwater is not None: self.play(FadeOut(self.curwater),FadeOut(self.pl),FadeOut(self.pr),run_time=0.3)
        lvl=min(H[self.bl],H[self.br]); w=self.water(self.bl,self.br,lvl)
        gw=glow(w,WATER,layers=6,spread=0.12,max_op=0.13)
        self.play(FadeIn(w),FadeIn(gw),
                  self.bars[self.bl].animate.set_stroke(MINT,4.6),self.bars[self.br].animate.set_stroke(MINT,4.6),
                  *self.hl(7),run_time=0.6)
        self.set_cap("lines 1 and 8: min(8,7) × 7 = 49",color=MINT)
        self.wait(READ_L)
        self.play(*self.clear_code(),run_time=0.4)
        ...
        badge=VGroup(cbox,comp).move_to(UP*-3.2)
    ...
    def end_slide(self):
        ...
        part=pill("28 / 150",MINT,SURFACE,MINT,s=0.44,h=0.62,glowing=True).move_to(UP*2.15)
