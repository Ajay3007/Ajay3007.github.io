...
S_Y=2.9; ROW_TOP=1.6; ROWSTEP=0.58; COLSTEP=0.6; GCELL=0.52
RES_Y=-0.55; CODE_Y=-3.2
READ=0.8; READ_L=1.4
...
def gcell(ch,color,bg):
    r=RoundedRectangle(width=GCELL,height=GCELL,corner_radius=0.09,stroke_color=color,
                       stroke_width=2.8,fill_color=bg,fill_opacity=1.0).set_z_index(1)
    t=Text(ch,font=FN,weight=BOLD,color=color).scale(0.55*GCELL).move_to(r.get_center()).set_z_index(5)
    return VGroup(r,t)
...
class LC6(Scene):
    ...
    def persistent(self):
        ...
        badge=pill("# 6",CYAN,SURFACE,CYAN,s=0.4)
        title=Text("Zigzag Conversion",font=FN,weight=BOLD,color=WHITE).scale(0.6)
    ...
    def problem_scene(self):
        ...
        t1=Text("Write the string in a ZIGZAG over numRows rows,",font=FN,color=WHITE).scale(0.42)
        t2=Text("then read it back row by row.",font=FN,color=GRAY).scale(0.42)
        for t in (t1,t2):
            if t.width>6.9: t.scale_to_fit_width(6.9)
        t1.move_to(UP*2.65); t2.next_to(t1,DOWN,buff=0.18)
        self.play(FadeIn(t1),run_time=0.45); self.play(FadeIn(t2),run_time=0.45)
        self.wait(0.5)
        # mini static zigzag preview
        pos=zigzag(S,NR)
        preview=VGroup()
        for (r,c),ch in zip(pos,S):
            preview.add(gcell(ch,ROWCOL[r],ROWBG[r]).move_to([(-6*0.5/2)+c*0.5,0.9-r*0.5,0]))
        preview.move_to(UP*0.6)
        self.play(LaggedStart(*[FadeIn(x,shift=DOWN*0.1) for x in preview],lag_ratio=0.04),run_time=1.2)
        self.wait(0.4)
        res=colorize('"PAHNAPLSIIGYIR"',0.44).move_to(UP*-1.7)
        self.play(FadeIn(res,shift=UP*0.1),run_time=0.5)
        self.wait(READ_L+0.2)
        self.play(FadeOut(VGroup(lbl,t1,t2,preview,res)),run_time=0.5)

    def hint_scene(self):
        lbl=pill("The key idea",AMBER,SURFACE,AMBER).move_to(UP*CAP_Y)
        h1=Text("Don't compute any coordinates.",font=FN,weight=BOLD,color=WHITE).scale(0.46)
        h2=Text("Keep one bucket of text per row.",font=FN,color=WHITE).scale(0.44)
        h3=Text("Drop each char into the current row,",font=FN,color=MINT).scale(0.44)
        h4=Text("bouncing direction at the top and bottom.",font=FN,color=CYAN).scale(0.44)
        h5=Text("Then just concatenate the rows.",font=FN,color=AMBER).scale(0.44)
    ...
    def approach_scene(self):
        ...
        a0=Text("One pass, bounce between rows",font=FN,weight=BOLD,color=WHITE).scale(0.5).move_to(UP*2.3)
        if a0.width>6.9: a0.scale_to_fit_width(6.9)
        a1=Text("rows[cur] += c  for each char",font=MN,color=WHITE).scale(0.4).move_to(UP*1.4)
        a2=Text("cur goes down… then flips up at the last row",font=FN,color=CYAN).scale(0.38).move_to(UP*0.7)
        a3=Text("…and flips back down at row 0",font=FN,color=MINT).scale(0.38).move_to(UP*0.1)
        a4=Text("answer = rows joined top to bottom",font=MN,color=AMBER).scale(0.38).move_to(UP*-0.65)
    ...
    def gx(self,c): return (-(self.ncols-1)*COLSTEP/2)+c*COLSTEP
    def gy(self,r): return ROW_TOP-r*ROWSTEP

    def walkthrough(self):
        pos=zigzag(S,NR); self.ncols=max(c for _,c in pos)+1
        # input string as per-char
        self.sch=[Text(ch,font=MN,weight=BOLD,color=WHITE).scale(0.5) for ch in S]
        srow=VGroup(*self.sch).arrange(RIGHT,buff=0.16).move_to(UP*S_Y)
        if srow.width>6.9: srow.scale_to_fit_width(6.9)
        slab=Text("s",font=MN,weight=BOLD,color=GRAY).scale(0.4).next_to(srow,LEFT,buff=0.2)
        self.play(FadeIn(slab),FadeIn(srow,shift=UP*0.1),run_time=0.7)

        rowlabs=VGroup(*[Text("row %d"%r,font=MN,weight=BOLD,color=ROWCOL[r]).scale(0.3).move_to([self.gx(0)-1.0,self.gy(r),0]) for r in range(NR)])
        self.rowlabs=rowlabs
        self.play(FadeIn(rowlabs),run_time=0.35)

        raw=[(0,'if (numRows == 1) return s;'),
             (0,'vector<string> rows(numRows);'),
             (0,'int cur = 0, down = -1;'),
             (0,'for (char c : s) {'),
             (1,'if (cur==0 || cur==numRows-1) down = -down;'),
             (1,'rows[cur] += c;   cur += down;'),
             (0,'}'),
             (0,'string res; for (auto& r : rows) res += r; return res;')]
        ...
        self.set_cap("fill the zigzag — bounce at top & bottom",color=WHITE)
        self.play(*self.hl(3),run_time=0.35)
        self.gcells={}
        for i,ch in enumerate(S):
            r,c=pos[i]; boundary=(r==0 or r==NR-1)
            g=gcell(ch,ROWCOL[r],ROWBG[r]).move_to([self.gx(c),self.gy(r),0])
            self.play(FadeIn(g,shift=(UP if r==0 else DOWN)*0.12),
                      self.sch[i].animate.set_color(GRAY),
                      *self.hl(4 if boundary else 5),run_time=0.28)
            self.gcells[(r,c)]=(g,ch)
        self.wait(0.3)

        # read row by row
        self.set_cap("read it back, one row at a time",color=WHITE)
        self.play(*self.hl(7),run_time=0.35)
        reslab=Text("res",font=MN,weight=BOLD,color=WHITE).scale(0.36).move_to([-2.7,RES_Y,0])
        self.play(FadeIn(reslab),run_time=0.3)
        chunks=VGroup(); prev=reslab
        for r in range(NR):
            rowcells=sorted([(c,g,ch) for (rr,c),(g,ch) in self.gcells.items() if rr==r])
            self.play(*[g.animate.set_fill(ROWBG[r],1.0).set_stroke(ROWCOL[r],4.6) for _,g,_ in rowcells],run_time=0.4)
            txt="".join(ch for _,_,ch in rowcells)
            chunk=Text(txt,font=MN,weight=BOLD,color=ROWCOL[r]).scale(0.5).next_to(prev,RIGHT,buff=0.14 if r==0 else 0.03)
            self.play(FadeIn(chunk,shift=UP*0.12),run_time=0.45)
            chunks.add(chunk); prev=chunk
            self.wait(0.15)
        self.result_group=VGroup(reslab,chunks)
        self.wait(READ_L)

    def outro(self):
        # clear the working parts, keep the assembled result
        self.play(FadeOut(self.hlbar),*[FadeOut(c) for c in self.code],FadeOut(self.panel),
                  FadeOut(VGroup(*self.sch)),*[FadeOut(g) for (g,_) in self.gcells.values()],
                  FadeOut(self.rowlabs),run_time=0.55)
        self.play(self.result_group.animate.move_to(UP*1.4),run_time=0.5)
        self.set_cap("each row is one colour — just glue them in order",color=MINT)
        self.wait(READ_L)
        ...
        sfx=Text("O(1) extra with direct index math per row",font=FN,color=GRAY).scale(0.34)
        badge=VGroup(cbox,comp).move_to(UP*-0.4)
    ...
    def end_slide(self):
        ...
        part=pill("22 / 150",MINT,SURFACE,MINT,s=0.44,h=0.62).move_to(UP*2.15)
