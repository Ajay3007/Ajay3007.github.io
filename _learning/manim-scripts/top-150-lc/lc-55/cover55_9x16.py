...
class Cover55(Scene):
    def construct(self):
        ...
        self.add(VGroup(pill("# 55",CYAN,SURFACE,CYAN),pill("Medium",MED_T,MED_B,MED_T)).arrange(RIGHT,buff=0.28).move_to(UP*5.1))
        title=Text("Jump Game",font=FN,weight=BOLD,color=WHITE)
        title.scale_to_fit_width(5.0).move_to(UP*3.95)
        self.add(title)
        ...
        sub=Text("nums[i] = max jump from index i",font=MN,color=GRAY).scale(0.36).move_to(UP*2.35)
        self.add(sub)
        vals=[3,2,1,0,4]
        row=VGroup()
        for i,v in enumerate(vals):
            if i==4: row.add(mcell(v,RED,RED_BG,RED,0.72))
            else: row.add(mcell(v,BORDER,SURFACE,WHITE,0.72))
        row.arrange(RIGHT,buff=0.16).move_to(UP*1.4)
        self.add(row,Text("nums",font=MN,weight=BOLD,color=WHITE).scale(0.36).next_to(row,LEFT,buff=0.24))
        idx=VGroup(*[Text(str(i),font=MN,color=GRAY).scale(0.32).next_to(row[i],DOWN,buff=0.16) for i in range(5)])
        self.add(idx)
        # reach bar covering 0..3
        left=row[0].get_left()[0]; right=row[3].get_right()[0]
        bar=RoundedRectangle(width=right-left,height=0.2,corner_radius=0.1,stroke_width=0,fill_color=MINT,fill_opacity=0.9)
        bar.move_to([(left+right)/2,0.35,0])
        self.add(bar,Text("reach",font=MN,weight=BOLD,color=MINT).scale(0.34).next_to(bar,LEFT,buff=0.24))
        self.add(Text("stalls at 3",font=MN,color=MINT).scale(0.32).next_to(bar,RIGHT,buff=0.2))

        hook=Text("greedy  \u00b7  track the farthest reach",font=FN,color=CYAN).scale(0.46)
        if hook.width>6.5: hook.scale_to_fit_width(6.5)
        hook.move_to(UP*-0.7); self.add(hook)

        box=RoundedRectangle(width=5.0,height=1.2,corner_radius=0.18,stroke_color=RED,stroke_width=3.2,fill_color=RED_BG,fill_opacity=1.0).move_to(UP*-2.05)
        bval=Text("index 4 unreachable = false",font=MN,weight=BOLD,color=RED).scale(0.46).move_to(box.get_center())
        if bval.width>4.6: bval.scale_to_fit_width(4.6)
        self.add(box,bval)
        self.add(Text("the 0 at index 3 is the wall",font=MN,color=GRAY).scale(0.36).next_to(box,DOWN,buff=0.24))
