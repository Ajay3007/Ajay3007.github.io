from manim import *
import re

BG="#0B0E14"; SURFACE="#151B26"; BORDER="#2A3542"
CYAN="#34D8E8"; AMBER="#F0A431"; WHITE="#E8ECF1"; GRAY="#5A6472"; MINT="#57E5B0"; RED="#F2544D"
CYAN_BG="#10303A"; AMBER_BG="#33240F"; RED_BG="#331617"; MINT_BG="#0F2A20"

# ---- v2 code-panel palette (syntax highlighting) ----
CODEFG="#C7D0DA"; KW_C=CYAN; NUM_C=AMBER; OP_C="#F78CA0"; STR_C=MINT; COM_C=GRAY
KEYWORDS=set("int long char bool void auto for while if else return const unsigned size_t struct".split())
TYPES=set("string vector pair map unordered_map stringstream".split())

config.pixel_width=1080
config.pixel_height=1920
config.frame_height=14.0
config.frame_width=7.875
config.background_color=BG
FN="DejaVu Sans"; MN="DejaVu Sans Mono"

WM_Y=5.05; HDR_Y=4.3; CAP_Y=3.55
HAY_Y=1.9; NDL_Y=0.95; SZ=0.72; STEP=0.86; RES_Y=-0.5; CODE_Y=-2.95
READ=0.8; READ_L=1.4

HAY="abcabd"; NDL="abd"

def _char_colors(s):
    col=[None]*len(s); ci=s.find("//")
    for m in re.finditer(r'[A-Za-z_]\w*|\d+|"[^"]*"|\'[^\']*\'|[-+*/%<>=!&|?:]+', s):
        tok=m.group(); a,b=m.start(),m.end()
        if ci!=-1 and a>=ci: continue
        if tok[0].isdigit(): c=NUM_C
        elif tok[0] in '"\'': c=STR_C
        elif tok[0] in "-+*/%<>=!&|?:": c=OP_C
        elif tok in KEYWORDS or tok in TYPES: c=KW_C
        else: c=None
        if c is not None:
            for k in range(a,b): col[k]=c
    if ci!=-1:
        for k in range(ci,len(s)): col[k]=COM_C
    return col

def colorize(s,scale=0.34):
    col=_char_colors(s); t2c={}; i=0
    while i<len(s):
        if col[i] is None: i+=1; continue
        j=i
        while j<len(s) and col[j]==col[i]: j+=1
        t2c["[%d:%d]"%(i,j)]=col[i]; i=j
    return Text(s,font=MN,color=CODEFG,t2c=t2c).scale(scale)

def pill(text,tc,bg,st,s=0.4,h=0.56):
    t=Text(text,font=MN,weight=BOLD,color=tc).scale(s)
    box=RoundedRectangle(width=t.width+0.45,height=h,corner_radius=h/2,stroke_color=st,stroke_width=2.4,fill_color=bg,fill_opacity=1.0)
    t.move_to(box.get_center()); return VGroup(box,t)

def cell(ch,size=SZ,stroke=BORDER,fill=SURFACE,tcolor=WHITE):
    r=RoundedRectangle(width=size,height=size,corner_radius=0.11,stroke_color=stroke,
                       stroke_width=3.0,fill_color=fill,fill_opacity=1.0).set_z_index(1)
    t=Text(ch,font=FN,weight=BOLD,color=tcolor).scale(0.58*size).move_to(r.get_center()).set_z_index(5)
    return VGroup(r,t)

def pointer(label,color,up=True):
    tri=Triangle(color=color,fill_opacity=1.0,stroke_width=0).scale(0.12)
    if up: tri.rotate(PI)
    lab=Text(label,font=MN,weight=BOLD,color=color).scale(0.4)
    lab.next_to(tri,UP if up else DOWN,buff=0.05)
    return VGroup(tri,lab).set_z_index(8)


class LC28(Scene):
    def construct(self):
        self.persistent()
        self.problem_scene()
        self.hint_scene()
        self.approach_scene()
        self.walkthrough()
        self.elegant_card()
        self.outro()
        self.end_slide()

    def persistent(self):
        axio=Text("Axio",font=FN,weight=BOLD,color=CYAN).scale(0.4)
        byte=Text("Byte",font=FN,weight=BOLD,color=AMBER).scale(0.4)
        self.wm=VGroup(axio,byte).arrange(RIGHT,buff=0.02).move_to(UP*WM_Y)
        badge=pill("# 28",CYAN,SURFACE,CYAN,s=0.4)
        title=Text("Find First Occurrence",font=FN,weight=BOLD,color=WHITE).scale(0.6)
        self.hdr=VGroup(badge,title).arrange(RIGHT,buff=0.3)
        if self.hdr.width>6.8: self.hdr.scale(6.8/self.hdr.width)
        self.hdr.move_to(UP*HDR_Y)
        self.play(FadeIn(self.wm,shift=DOWN*0.15),run_time=0.5)
        self.play(FadeIn(self.hdr[0],shift=RIGHT*0.2),Write(self.hdr[1]),run_time=0.9)
        self.wait(0.3); self.cap=None

    def set_cap(self,txt,color=WHITE,scale=0.46,rt=0.45):
        new=Text(txt,font=FN,color=color).scale(scale)
        if new.width>6.7: new.scale_to_fit_width(6.7)
        new.move_to(UP*CAP_Y)
        if self.cap is None:
            self.cap=new; self.play(FadeIn(new,shift=UP*0.1),run_time=rt); return
        self.play(FadeOut(self.cap,shift=UP*0.08),run_time=0.22)
        self.play(FadeIn(new,shift=UP*0.08),run_time=0.32); self.cap=new

    # ---------- v2 code panel: syntax highlighting + active-line bar ----------
    def build_code(self,raw,scale=0.32,cy=CODE_Y,maxw=6.6):
        lines=[colorize(s,scale) for _,s in raw]
        block=VGroup(*lines).arrange(DOWN,aligned_edge=LEFT,buff=0.12)
        for ln,(ind,_) in zip(lines,raw): ln.shift(RIGHT*ind*0.30)
        if block.width>maxw: block.scale(maxw/block.width)
        panel=RoundedRectangle(corner_radius=0.14,stroke_color=BORDER,stroke_width=2.3,
                               fill_color="#0F1420",fill_opacity=1.0,
                               width=block.width+0.6,height=block.height+0.44).move_to(UP*cy).set_z_index(0)
        block.move_to(panel.get_center()); block.set_z_index(2)
        hlbar=RoundedRectangle(width=block.width+0.36,height=lines[0].height+0.12,corner_radius=0.07,
                               stroke_width=0,fill_color=CYAN,fill_opacity=0.0).set_z_index(1)
        hlbar.move_to([panel.get_center()[0],lines[0].get_center()[1],0])
        self.panel=panel; self.code=lines; self.hlbar=hlbar; self._barvis=False
        return panel,lines,hlbar

    def hl(self,i):
        target=[self.panel.get_center()[0],self.code[i].get_center()[1],0]
        if not self._barvis:
            self.hlbar.move_to(target); self._barvis=True
            return [self.hlbar.animate.set_fill(CYAN,opacity=0.15)]
        return [self.hlbar.animate.move_to(target)]

    def col_x(self,c):
        n=len(HAY); x0=-((n-1)*STEP)/2
        return x0+c*STEP

    def problem_scene(self):
        lbl=pill("The problem",CYAN,SURFACE,CYAN).move_to(UP*CAP_Y)
        self.play(FadeIn(lbl,shift=UP*0.1),run_time=0.5)
        t1=Text("Find the first index where needle appears in haystack.",font=FN,color=WHITE).scale(0.4)
        t2=Text("Return that index, or -1 if it never does.",font=FN,color=GRAY).scale(0.4)
        for t in (t1,t2):
            if t.width>6.9: t.scale_to_fit_width(6.9)
        t1.move_to(UP*2.65); t2.next_to(t1,DOWN,buff=0.2)
        self.play(FadeIn(t1),run_time=0.45); self.play(FadeIn(t2),run_time=0.45)
        self.wait(0.5)
        hrow=VGroup(*[cell(ch) for ch in HAY]).arrange(RIGHT,buff=STEP-SZ).move_to(UP*0.9)
        hlbl=Text("haystack",font=MN,weight=BOLD,color=WHITE).scale(0.34).next_to(hrow,LEFT,buff=0.24)
        self.play(FadeIn(hlbl),LaggedStart(*[FadeIn(c,shift=UP*0.1) for c in hrow],lag_ratio=0.08),run_time=1.0)
        nrow=VGroup(*[cell(ch,tcolor=CYAN) for ch in NDL]).arrange(RIGHT,buff=STEP-SZ).next_to(hrow,DOWN,buff=0.5)
        nlbl=Text("needle",font=MN,weight=BOLD,color=CYAN).scale(0.34).next_to(nrow,LEFT,buff=0.24)
        self.play(FadeIn(nlbl),LaggedStart(*[FadeIn(c,shift=UP*0.1) for c in nrow],lag_ratio=0.08),run_time=0.8)
        self.wait(READ_L)
        self.play(FadeOut(VGroup(lbl,t1,t2,hrow,hlbl,nrow,nlbl)),run_time=0.5)

    def hint_scene(self):
        lbl=pill("The key idea",AMBER,SURFACE,AMBER).move_to(UP*CAP_Y)
        h1=Text("Slide the needle across the haystack.",font=FN,weight=BOLD,color=WHITE).scale(0.46)
        h2=Text("At each start, compare left to right.",font=FN,color=WHITE).scale(0.44)
        h3=Text("All characters match → that's the index.",font=FN,color=MINT).scale(0.44)
        h4=Text("Any mismatch → shift over by one and retry.",font=FN,color=RED).scale(0.44)
        for h in (h1,h2,h3,h4):
            if h.width>6.8: h.scale_to_fit_width(6.8)
        h1.move_to(UP*2.0); h2.next_to(h1,DOWN,buff=0.3)
        h3.next_to(h2,DOWN,buff=0.3); h4.next_to(h3,DOWN,buff=0.24)
        self.play(FadeIn(lbl,shift=UP*0.1),run_time=0.5)
        self.play(FadeIn(h1,shift=UP*0.1),run_time=0.5); self.wait(0.2)
        self.play(FadeIn(h2,shift=UP*0.1),run_time=0.42)
        self.play(FadeIn(h3,shift=UP*0.1),run_time=0.42); self.play(FadeIn(h4,shift=UP*0.1),run_time=0.42)
        self.wait(READ_L+0.4)
        self.play(FadeOut(VGroup(lbl,h1,h2,h3,h4)),run_time=0.5)

    def approach_scene(self):
        lbl=pill("Approach",MINT,SURFACE,MINT).move_to(UP*CAP_Y)
        a0=Text("Sliding window scan",font=FN,weight=BOLD,color=WHITE).scale(0.54).move_to(UP*2.3)
        a1=Text("try each start i from 0 to n - m",font=MN,color=WHITE).scale(0.4).move_to(UP*1.4)
        a2=Text("match needle[k] against haystack[i+k]",font=MN,color=CYAN).scale(0.38).move_to(UP*0.7)
        a3=Text("all m matched → return i", font=MN,color=MINT).scale(0.4).move_to(UP*0.05)
        a4=Text("fell through everything → return -1",font=MN,color=RED).scale(0.38).move_to(UP*-0.65)
        for g in (a1,a2,a3,a4):
            if g.width>6.9: g.scale_to_fit_width(6.9)
        self.play(FadeIn(lbl,shift=UP*0.1),run_time=0.5)
        self.play(FadeIn(a0,shift=UP*0.1),run_time=0.5)
        self.play(FadeIn(a1,shift=UP*0.1),run_time=0.4)
        self.play(FadeIn(a2,shift=UP*0.1),run_time=0.42); self.wait(0.2)
        self.play(FadeIn(a3,shift=UP*0.1),run_time=0.42)
        self.play(FadeIn(a4,shift=UP*0.1),run_time=0.4)
        self.wait(READ_L+0.4)
        self.play(FadeOut(VGroup(lbl,a0,a1,a2,a3,a4)),run_time=0.5)

    def walkthrough(self):
        n=len(HAY); m=len(NDL)
        self.hay=[cell(ch) for ch in HAY]
        for c,cellobj in enumerate(self.hay): cellobj.move_to([self.col_x(c),HAY_Y,0])
        hlbl=Text("haystack",font=MN,weight=BOLD,color=WHITE).scale(0.32).move_to([self.col_x(0)-1.15,HAY_Y,0])
        idx=VGroup(*[Text(str(c),font=MN,color=GRAY).scale(0.3).next_to(self.hay[c],DOWN,buff=0.12) for c in range(n)])
        self.ndl=[cell(ch,tcolor=CYAN,stroke=CYAN) for ch in NDL]
        for k,cellobj in enumerate(self.ndl): cellobj.move_to([self.col_x(k),NDL_Y,0])
        nlbl=Text("needle",font=MN,weight=BOLD,color=CYAN).scale(0.32).move_to([self.col_x(0)-1.15,NDL_Y,0])
        self.play(FadeIn(hlbl),LaggedStart(*[FadeIn(c,shift=UP*0.1) for c in self.hay],lag_ratio=0.06),FadeIn(idx),run_time=0.9)
        self.play(FadeIn(nlbl),LaggedStart(*[FadeIn(c,shift=UP*0.1) for c in self.ndl],lag_ratio=0.06),run_time=0.7)

        raw=[(0,'int n = haystack.size(), m = needle.size();'),
             (0,'for (int i = 0; i + m <= n; i++) {'),
             (1,'int k = 0;'),
             (1,'while (k < m && haystack[i+k] == needle[k]) k++;'),
             (1,'if (k == m) return i;'),
             (0,'}'),
             (0,'return -1;')]
        panel,code,hlbar=self.build_code(raw)
        self._act=[]
        self.play(FadeIn(panel),run_time=0.35); self.add(hlbar)
        self.play(LaggedStart(*[FadeIn(l,shift=RIGHT*0.06) for l in code],lag_ratio=0.05),run_time=0.85)

        self.pi=pointer("i",AMBER,up=True).next_to(self.hay[0],UP,buff=0.12)
        self.ndlgrp=VGroup(*self.ndl)
        self.play(FadeIn(self.pi,shift=DOWN*0.12),*self.hl(1),run_time=0.45)

        found=-1
        for i in range(n-m+1):
            # slide needle window to start i
            target=[self.col_x(i+(m-1)/2),NDL_Y,0]
            self.play(self.ndlgrp.animate.move_to(target),
                      self.pi.animate.next_to(self.hay[i],UP,buff=0.12),*self.hl(3),run_time=0.4)
            touched=[]; ok=True
            for k in range(m):
                same=(HAY[i+k]==NDL[k])
                col=MINT if same else RED
                bg=MINT_BG if same else RED_BG
                self.set_cap("i=%d, k=%d : '%s' %s '%s'" % (i,k,HAY[i+k],"=" if same else "≠",NDL[k]),
                             color=col,scale=0.44)
                self.play(self.hay[i+k][0].animate.set_stroke(col,4.8).set_fill(bg,1.0),
                          self.ndl[k][0].animate.set_stroke(col,4.8).set_fill(bg,1.0),run_time=0.34)
                touched+=[self.hay[i+k],self.ndl[k]]
                if not same: ok=False; break
            if ok:
                found=i
                self.play(*self.hl(4),run_time=0.35)
                break
            # reset touched haystack cells (needle keeps its cyan identity)
            self.play(*[h[0].animate.set_stroke(BORDER,3.0).set_fill(SURFACE,1.0) for h in touched if h in self.hay],
                      *[nd[0].animate.set_stroke(CYAN,3.0).set_fill(SURFACE,1.0) for nd in touched if nd in self.ndl],
                      run_time=0.3)
        self.found=found
        self.wait(READ_L)

    def elegant_card(self):
        self.play(FadeOut(self.hlbar),*[FadeOut(c) for c in self.code],FadeOut(self.panel),run_time=0.45)
        tag=pill("Bonus: cleaner code  ✨",AMBER,AMBER_BG,AMBER,s=0.42,h=0.62).move_to(UP*RES_Y+UP*0.15)
        self.play(FadeIn(tag,scale=1.05),run_time=0.5)
        raw=[(0,'size_t p = haystack.find(needle);'),
             (0,'return p == string::npos ? -1 : (int)p;')]
        panel,code,_=self.build_code(raw,scale=0.34,cy=-2.1,maxw=6.7)
        self.play(FadeIn(panel),run_time=0.35)
        self.play(LaggedStart(*[FadeIn(l,shift=RIGHT*0.06) for l in code],lag_ratio=0.08),run_time=0.8)
        note=Text("string::find already does the scan — npos means not found",font=FN,color=GRAY).scale(0.34).move_to(UP*-3.5)
        if note.width>6.9: note.scale_to_fit_width(6.9)
        self.play(FadeIn(note,shift=UP*0.1),run_time=0.45)
        self.set_cap("or let the library do it",color=AMBER)
        self.wait(READ_L+0.2)
        self.elegant=VGroup(tag,panel,*code,note)

    def outro(self):
        self.play(FadeOut(self.elegant),run_time=0.45)
        ans=pill("first occurrence  →  index %d" % self.found,MINT,MINT_BG,MINT,s=0.48,h=0.72).move_to(UP*RES_Y)
        self.play(FadeIn(ans,scale=1.05),run_time=0.5)
        self.set_cap("matched at i = %d — that's the answer" % self.found,color=MINT)
        self.wait(READ_L)
        comp=VGroup(Text("Time  O(n·m)",font=MN,weight=BOLD,color=CYAN).scale(0.48),
                    Text("Space  O(1)",font=MN,weight=BOLD,color=AMBER).scale(0.48)).arrange(RIGHT,buff=0.45)
        cbox=RoundedRectangle(corner_radius=0.16,stroke_color=BORDER,stroke_width=2.4,fill_color="#0F1420",fill_opacity=1.0,width=comp.width+0.7,height=comp.height+0.5)
        comp.move_to(cbox.get_center())
        sfx=Text("O(n+m) with KMP",font=FN,color=GRAY).scale(0.34)
        badge=VGroup(cbox,comp).move_to(UP*-1.65)
        sfx.next_to(badge,DOWN,buff=0.22)
        self.play(FadeIn(badge,shift=UP*0.15),run_time=0.5)
        self.play(FadeIn(sfx),run_time=0.35); self.wait(READ_L)

    def end_slide(self):
        self.play(*[FadeOut(m) for m in self.mobjects],run_time=0.55)
        axio=Text("Axio",font=FN,weight=BOLD,color=CYAN).scale(0.8)
        byte=Text("Byte",font=FN,weight=BOLD,color=AMBER).scale(0.8)
        wm=VGroup(axio,byte).arrange(RIGHT,buff=0.03).move_to(UP*3.4)
        part=pill("23 / 150",MINT,SURFACE,MINT,s=0.44,h=0.62).move_to(UP*2.15)
        head=Text("LeetCode Top Interview 150",font=FN,weight=BOLD,color=WHITE).scale(0.58)
        if head.width>6.6: head.scale_to_fit_width(6.6)
        head.move_to(UP*1.2)
        sub=Text("every problem, visually explained",font=FN,color=GRAY).scale(0.46).move_to(UP*0.5)
        dl=Line(LEFT*1.6,ORIGIN,color=CYAN,stroke_width=4).move_to(UP*-0.15+LEFT*0.8)
        dr=Line(ORIGIN,RIGHT*1.6,color=AMBER,stroke_width=4).move_to(UP*-0.15+RIGHT*0.8)
        ctatxt=Text("Follow  @axiobyte",font=FN,weight=BOLD,color=CYAN).scale(0.6)
        ctabox=RoundedRectangle(width=ctatxt.width+0.9,height=1.0,corner_radius=0.5,stroke_color=CYAN,stroke_width=3.0,fill_color=CYAN_BG,fill_opacity=1.0)
        ctatxt.move_to(ctabox.get_center()); cta=VGroup(ctabox,ctatxt).move_to(UP*-1.05)
        nxt=VGroup(Text("↓",font=FN,weight=BOLD,color=AMBER).scale(0.55),
                   Text("comment the next problem",font=FN,color=WHITE).scale(0.46)).arrange(RIGHT,buff=0.2).move_to(UP*-2.35)
        self.play(FadeIn(wm,shift=DOWN*0.2),run_time=0.6)
        self.play(FadeIn(part,shift=UP*0.1),run_time=0.35)
        self.play(FadeIn(head,shift=UP*0.1),FadeIn(sub,shift=UP*0.1),run_time=0.55)
        self.play(Create(dl),Create(dr),run_time=0.4)
        self.play(FadeIn(cta,scale=1.06),run_time=0.55)
        self.play(FadeIn(nxt,shift=UP*0.1),run_time=0.45)
        self.wait(READ_L+0.8)
