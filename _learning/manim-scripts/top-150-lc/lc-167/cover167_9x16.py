...
NUMS=[1,3,4,5,7,11]; TARGET=9; SZ=0.8; STEP=0.98
ANS={2,3}   # 0-based indices of the matching pair (4 + 5 = 9)
...
def mcell(v,stroke,fill,tcolor,size,glowc=None):
    ...
    t=Text(str(v),font=FN,weight=BOLD,color=tcolor).scale(0.56*size).move_to(r.get_center()).set_z_index(3)
    g=VGroup(r,t)
    if glowc is not None: g.add_to_back(glow(r,glowc))
    return g

class Cover167(Scene):
    def construct(self):
        ...
        self.add(VGroup(pill("# 167",CYAN,SURFACE,CYAN),pill("Medium",MED_T,MED_B,MED_T)).arrange(RIGHT,buff=0.28).move_to(UP*5.1))
        title=Text("Two Sum II — Sorted",font=FN,weight=BOLD,color=WHITE)
        ...
        sub=Text("both ends · sum too small → l++ · too big → r--",font=MN,color=GRAY).scale(0.32).move_to(UP*2.45)
        if sub.width>6.6: sub.scale_to_fit_width(6.6)
        self.add(sub)

        x0=-((len(NUMS)-1)*STEP)/2; row_y=1.4
        for c,v in enumerate(NUMS):
            if c in ANS: st,fl,tc,gc=MINT,MINT_BG,MINT,MINT
            else: st,fl,tc,gc=BORDER,SURFACE,WHITE,None
            self.add(mcell(v,st,fl,tc,SZ,glowc=gc).move_to([x0+c*STEP,row_y,0]))
        idx=VGroup(*[Text(str(c+1),font=MN,color=GRAY).scale(0.32).move_to([x0+c*STEP,row_y-SZ/2-0.3,0]) for c in range(len(NUMS))])
        self.add(idx)
        # pointers at the ends
        self.add(Text("l",font=MN,weight=BOLD,color=CYAN).scale(0.42).move_to([x0,row_y+SZ/2+0.32,0]))
        self.add(Text("r",font=MN,weight=BOLD,color=AMBER).scale(0.42).move_to([x0+(len(NUMS)-1)*STEP,row_y+SZ/2+0.32,0]))

        self.add(pill("target = %d"%TARGET,AMBER,AMBER_BG,AMBER,s=0.46,glowing=True).move_to(UP*-0.15))

        box=RoundedRectangle(width=5.4,height=1.2,corner_radius=0.18,stroke_color=MINT,stroke_width=3.2,fill_color=MINT_BG,fill_opacity=1.0).move_to(UP*-1.75)
        box.add_to_back(glow(box,MINT,layers=6,spread=0.28,max_op=0.14))
        bval=Text("4 + 5 = 9  →  [3, 4]",font=MN,weight=BOLD,color=MINT).scale(0.5).move_to(UP*-1.75)
        if bval.width>5.0: bval.scale_to_fit_width(5.0)
        self.add(box,bval)
        self.add(Text("1-based indices",font=MN,color=GRAY).scale(0.34).move_to(UP*-2.7))
