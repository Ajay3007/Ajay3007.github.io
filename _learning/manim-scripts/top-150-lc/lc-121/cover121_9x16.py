...
class Cover121(Scene):
    def construct(self):
        ...
        self.add(VGroup(pill("# 121",CYAN,SURFACE,CYAN),pill("Easy",GREEN_T,GREEN_B,GREEN_T)).arrange(RIGHT,buff=0.28).move_to(UP*5.1))
        title=Text("Best Time to Buy/Sell Stock",font=FN,weight=BOLD,color=WHITE)
        ...
        for i,v in enumerate(vals):
            if i==1: row.add(mcell(v,AMBER,AMBER_BG,WHITE,0.66))
            elif i==4: row.add(mcell(v,MINT,MINT_BG,WHITE,0.66))
            else: row.add(mcell(v,BORDER,SURFACE,WHITE,0.66))
        row.arrange(RIGHT,buff=0.12).move_to(UP*1.9)
        self.add(row,Text("prices",font=MN,weight=BOLD,color=WHITE).scale(0.38).next_to(row,LEFT,buff=0.24))
        self.add(Text("buy @ 1",font=MN,weight=BOLD,color=AMBER).scale(0.34).next_to(row[1],DOWN,buff=0.2))
        self.add(Text("sell @ 6",font=MN,weight=BOLD,color=MINT).scale(0.34).next_to(row[4],DOWN,buff=0.2))

        self.add(Arrow(start=[0,0.3,0],end=[0,-0.3,0],buff=0,color=CYAN,stroke_width=6,max_tip_length_to_length_ratio=0.4).move_to(UP*0.45))

        hook=Text("one pass  \u00b7  track the low, sell high",font=FN,color=CYAN).scale(0.46)
        if hook.width>6.5: hook.scale_to_fit_width(6.5)
        hook.move_to(UP*-0.35); self.add(hook)

        box=RoundedRectangle(width=4.2,height=1.2,corner_radius=0.18,stroke_color=MINT,stroke_width=3.2,fill_color=MINT_BG,fill_opacity=1.0).move_to(UP*-1.75)
        bval=Text("max profit = 5",font=MN,weight=BOLD,color=MINT).scale(0.6).move_to(box.get_center())
        self.add(box,bval)
        self.add(Text("buy 1  \u2192  sell 6",font=MN,color=GRAY).scale(0.38).next_to(box,DOWN,buff=0.24))

        self.add(Text("O(n) time      \u00b7      O(1) space",font=MN,weight=BOLD,color=WHITE).scale(0.44).move_to(UP*-3.9))
