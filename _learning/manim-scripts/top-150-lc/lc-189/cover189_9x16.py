...
class Cover189(Scene):
    def construct(self):
        ...
        self.add(VGroup(pill("# 189",CYAN,SURFACE,CYAN),pill("Medium",MED_T,MED_B,MED_T)).arrange(RIGHT,buff=0.28).move_to(UP*5.1))
        title=Text("Rotate Array",font=FN,weight=BOLD,color=WHITE)
        ...
        kchip=pill("k = 2",AMBER,SURFACE,AMBER,s=0.4).move_to(UP*2.3)
        self.add(kchip)
        invals=[1,2,3,4,5]
        inrow=VGroup()
        for i,v in enumerate(invals):
            if i>=3: inrow.add(mcell(v,CYAN,CYAN_BG,WHITE,0.64))
            else: inrow.add(mcell(v,BORDER,SURFACE,WHITE,0.64))
        inrow.arrange(RIGHT,buff=0.12).move_to(UP*1.5)
        self.add(inrow,Text("nums",font=MN,weight=BOLD,color=WHITE).scale(0.38).next_to(inrow,LEFT,buff=0.26))
        ...
        hook=Text("3 reversals  \u00b7  rotate in place",font=FN,color=CYAN).scale(0.48)
        ...
        for i,v in enumerate(outvals):
            if i<2: res.add(mcell(v,MINT,MINT_BG,MINT,0.9))
            else: res.add(mcell(v,GRAY,SURFACE,WHITE,0.9))
        res.arrange(RIGHT,buff=0.12).move_to(UP*-1.6)
        self.add(res)
        self.add(Text("rotated right by 2",font=MN,color=GRAY).scale(0.38).next_to(res,DOWN,buff=0.22))
